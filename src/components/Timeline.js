...
function buildMonthSegments(startDate, endDate) {
    const segs = [];
...
export const Timeline = ({ startDate, endDate, dayWidth, unit }) => {
    ...
    const topRow = useMemo(() => {
        return monthSegs.map((seg, idx) => {
            ...
            return (_jsx("div", { className: "flex items-center justify-center text-xs", style: {
                    width,
                    height: H_TOP,
                    background: (((seg.labelYear * 12 + seg.labelMonth) % 2) === 0) ? MONTH_BG_A : MONTH_BG_B,
                    borderLeft
                }, children: text }, `m-${seg.labelYear}-${seg.labelMonth}-${startX}`));
        });
    ...
    const bottomRow = useMemo(() => {
        if (unit === 'day') {
            ...
            return Array.from({ length: total }).map((_, i) => {
                ...
                return (_jsx("div", { className: "absolute flex items-center justify-center text-xs", style: {
        ...
        if (unit === 'week') {
            ...
            while (cur <= endDate) {
                ...
                const w = 7 * dayWidth;
                nodes.push(_jsx("div", { className: "absolute flex items-center justify-center text-xs", style: {
                        transform: `translateX(${x}px)`,
                        width: w,
                        height: H_BOTTOM,
                        top: H_TOP,
                        borderLeft: `1px solid ${THIN_LINE}`
                    }, children: `${ws.getDate()}–${we.getDate()} (${getISOWeekNumber(ws)}н)` }, `wk-${ws.toISOString()}`));
                cur = addDays(cur, 7);
            }
            return nodes;
        }
    ...
    return (_jsxs("div", { className: "relative w-full select-none", style: { height: HEADER_H, pointerEvents: 'none' }, children: [_jsx("div", { className: "flex", style: { height: H_TOP }, children: topRow }), _jsx("div", { style: { position: 'absolute', left: 0, right: 0, top: H_TOP - 0.5, height: 0, borderTop: `1px solid ${BORDER_BOTTOM}` } }), _jsx("div", { className: "absolute left-0 right-0", style: { top: 0, height: HEADER_H }, children: bottomRow })] }));
};
