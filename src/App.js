...
export default function App() {
    const [project, setProject] = useState(() => loadProject() ?? initialProject);
    const [unit, setUnit] = useState('day');
    const [openNewTask, setOpenNewTask] = useState(false);
    const [openNewMilestone, setOpenNewMilestone] = useState(false);
    const [editingTask, setEditingTask] = useState(null);
    const [editingMilestone, setEditingMilestone] = useState(null);
    const handleCreateMilestone = (payload) => {
    ...
    const applyProject = (next, pushHist = true) => {
        if (pushHist)
            pushHistory(project);
        setProject(next);
        saveProject(next);
    };
    const onUpdateProject = (next) => applyProject(next, true);
    useEffect(() => {
        const handler = (e) => {
    ...
    const handleSaveMilestone = (payload) => {
        if (editingMilestone) {
    ...
    return (_jsxs("div", { className: "h-screen flex flex-col bg-background", children: [_jsx(ProjectHeader, { project: project, onUpdateProject: (p) => applyProject(p, true), unit: unit, onChangeUnit: setUnit }), _jsxs("div", { className: "flex-1 overflow-hidden", children: [_jsx(GanttChart, { project: project, onUpdateProject: onUpdateProject, unit: unit, onEditTask: handleStartEditTask, onEditMilestone: handleStartEditMilestone }), _jsx("div", { className: "fixed bottom-6 right-6", children: _jsx(CreateItemMenu, { onCreateTask: () => setOpenNewTask(true), onCreateMilestone: () => setOpenNewMilestone(true) }) }), _jsx(NewTaskDialog, { open: openNewTask, onClose: () => { setOpenNewTask(false); setEditingTask(null); }, onCreate: handleSaveTask, task: editingTask ?? undefined }), _jsx(MilestoneForm, { isOpen: openNewMilestone, onClose: () => { setOpenNewMilestone(false); setEditingMilestone(null); }, milestone: editingMilestone ?? undefined, onSave: handleSaveMilestone })] })] }));
}
